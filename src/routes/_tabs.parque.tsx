...
import { MapPin, Clock, Zap, Star, Sparkles } from "lucide-react";
...
function ParquePage() {
  ...
  const { data: day } = useQuery({
    queryKey: ["day-today", today],
    queryFn: async () => {
      const { data: exact } = await supabase
        .from("days")
        .select("*, park:parks(*), itinerary_steps(*, attraction:attractions(*))")
        .eq("date", today)
        .maybeSingle();
  ...
  if (!day) {
    return (
      <div className="px-5 pt-10 text-center text-muted-foreground">
        Nenhum dia/parque cadastrado.
      </div>
    );
  }

  const park = day.park;
  const steps = (day.itinerary_steps ?? []).slice().sort((a: any, b: any) => a.position - b.position);
    ...
    <div className="px-5 pt-8">
      <header>
        <div className="text-xs uppercase tracking-[0.2em] text-primary font-semibold">
          {isToday ? "Parque de hoje" : "Próximo parque"}
        </div>
        <h1 className="mt-2 text-3xl font-bold">{park?.name ?? day.title}</h1>
      ...
      {park && (
        <div className="mt-5 rounded-2xl border border-border bg-card p-4 space-y-2">
          {park.hours && (
            <p className="text-sm flex items-center gap-2"><Clock className="h-4 w-4 text-primary" />{park.hours}</p>
          )}
          {park.address && (
            <a
            ...
            >
              <MapPin className="h-4 w-4" />{park.address}
            </a>
          )}
          {park.arrival_tip && (
      ...
      <h2 className="mt-8 text-sm uppercase tracking-wider text-muted-foreground font-semibold flex items-center gap-2">
        <Sparkles className="h-4 w-4 text-primary" /> Ordem do dia
      </h2>
      ...
      <ol className="mt-3 space-y-3">
        {steps.map((s: any, i: number) => {
          ...
          return (
            <li
              key={s.id}
              className={`rounded-2xl border bg-card p-4 ${s.is_key_moment ? "border-primary" : "border-border"}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    {s.time && <span className="text-xs font-mono text-primary">{s.time}</span>}
                    {s.is_key_moment && <Star className="h-3.5 w-3.5 text-primary fill-primary" />}
                  </div>
                  <h3 className="font-semibold mt-0.5">{s.label}</h3>
                  {s.note && <p className="text-xs text-muted-foreground mt-1">{s.note}</p>}
                  {a && (
                    <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[11px]">
                      {a.area && <span className="px-2 py-0.5 rounded-full bg-secondary">{a.area}</span>}
                      {a.duration && <span className="px-2 py-0.5 rounded-full bg-secondary">⏱ {a.duration}</span>}
                      {a.estimated_wait && <span className="px-2 py-0.5 rounded-full bg-secondary">fila ~{a.estimated_wait}</span>}
                      {a.lightning_lane && (
                      ...
                      {a.must_do && <span className="px-2 py-0.5 rounded-full bg-primary text-primary-foreground font-semibold">MUST</span>}
                    </div>
                  )}
                  {a?.strategy_tip && (
                    <p className="text-xs text-muted-foreground border-l-2 border-primary pl-3 mt-2">
      ...
      {steps.length > 0 && (
        <div className="mt-6 rounded-2xl border border-border bg-card overflow-hidden">
          <iframe
            ...
            src={`https://www.google.com/maps?q=${encodeURIComponent(park?.address || park?.name || "Orlando FL")}&output=embed`}
          />
        </div>
      )}
    </div>
